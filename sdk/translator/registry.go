...
func (r *Registry) TranslateNonStream(ctx context.Context, from, to Format, model string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) []byte {
	r.mu.RLock()
	var nonStream ResponseNonStreamTransform
	if byTarget, ok := r.responses[to]; ok {
		nonStream = byTarget[from].NonStream
	}
	hooks := r.hooks
	r.mu.RUnlock()
	...
	if nonStream != nil {
		usedNativeTransform = true
		result = nonStream(ctx, model, originalRequestRawJSON, requestRawJSON, body, param)
	} else if hooks != nil {
		if translated, ok := hooks.TranslateResponse(ctx, from, to, model, originalRequestRawJSON, requestRawJSON, body, false); ok {
			result = translated
		}
	}
	if result == nil && !usedNativeTransform {
		result = body
	}
	if hooks != nil {
		result = hooks.NormalizeResponseAfter(ctx, from, to, model, originalRequestRawJSON, requestRawJSON, result, false)
	}
	return result
}
