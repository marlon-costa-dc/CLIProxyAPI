...
func ComputeOpenAICompatModelsHash(models []config.OpenAICompatibilityModel) string {
	keys := modelRoutingKeys(func(out func(key string)) {
		for _, model := range models {
			...
			out(strings.ToLower(name) + "|" + strings.ToLower(alias) + "|" + strings.TrimSpace(model.DisplayName) + "|" + fmt.Sprintf("image=%t", model.Image) + "|" + fmt.Sprintf("force-mapping=%t", model.ForceMapping) + "|" + fmt.Sprintf("is-compat=%t", model.IsCompat) + "|input=" + strings.Join(normalizeModalities(model.InputModalities), ",") + "|output=" + strings.Join(normalizeModalities(model.OutputModalities), ",") + "|catalog-provider=" + model.CatalogProviderID + "|catalog-model=" + model.CatalogModelID + "|catalog-route-provider=" + model.CatalogRouteProviderID + "|catalog-route-model=" + model.CatalogRouteModelID + "|variant=" + model.VariantID + "|protocols=" + strings.Join(model.Protocols, ",") + thinkingHashSuffix(model.Thinking))
		}
	})
